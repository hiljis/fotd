...
const sizeToXCoord = (size: 16 | 24 | 32 | 42 | 64 | 128) => {
	if (size === 16) return 1;
	else if (size === 24) return 3;
	else if (size === 32) return 1;
	else if (size === 42) return 1;
...
const HeaderCartoon: React.FC<Props> = ({ size, lineOne, lineTwo, uppercase }) => {
	...
	const xCoord = sizeToXCoord(size);
			...
			<div className={`headerCartoon__container size--${size}`}>
				<h3 className={`headerCartoon size--${size}`}>
					<svg xmlns="http://www.w3.org/2000/svg" className={`size--${size}`}>
						<text x={xCoord} y={yCoord} className={`size--${size} ${uppercase ? 'uppercase' : ''}`}>
							{lineOne}
						</text>
					</svg>
					<svg xmlns="http://www.w3.org/2000/svg" className={`shadow ${`size--${size}`}`}>
						<text x={xCoord} y={yCoord} className={`size--${size} shadow ${uppercase ? 'uppercase' : ''}`}>
							{lineOne}
						</text>
					</svg>
				</h3>
